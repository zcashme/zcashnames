...
import DraftEditor from "@/components/admin/beta-v2/DraftEditor";
import { contactValueFor, getOrCreateDraft, listDraftTesters } from "@/lib/beta-v2/drafts";
...
export default async function DraftEditorPage({
  ...
  const previewHtml = await renderBetaInvitePreview({
    ...
    walletVariantId: tester.preferred_wallet_variant_id,
  });
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="text-xs text-zinc-500">
        ...
        <Link href="/admin/beta-v2/drafts" className="text-amber-400 hover:text-amber-300">
          Back to list
        </Link>
      </div>
      <DraftEditor
        ...
        prevId={prevId}
        nextId={nextId}
        draftsListHref="/admin/beta-v2/drafts"
...
function TesterNotInDrafts({ id }: { id: string }) {
  return (
    <section className="rounded-md border border-zinc-800 bg-zinc-900/40 p-8 text-sm text-zinc-300">
      <p className="mb-2">
        Tester <code className="text-amber-400">{id}</code> is not in the current beta drafts list.
      </p>
      <Link href="/admin/beta-v2/drafts" className="text-amber-400 hover:text-amber-300">
        Back to drafts
      </Link>
...
function TesterMissingContact({ id }: { id: string }) {
  return (
    <section className="rounded-md border border-zinc-800 bg-zinc-900/40 p-8 text-sm text-zinc-300">
      <p className="mb-2">
        Tester <code className="text-amber-400">{id}</code> has no contact value for their preferred contact method.
      </p>
      <Link href="/admin/beta-v2/drafts" className="text-amber-400 hover:text-amber-300">
        Back to drafts
      </Link>
