...
import { getOrCreateDraft, updateDraft } from "@/lib/beta-v2/drafts";
import { readPreferredWalletVariantId } from "@/lib/beta/wallet-selection";
import { renderBetaInvitePreview, sendBetaInviteEmail } from "@/lib/email/beta-invite";
...
async function loadTester(testerId: string): Promise<TesterForSend | null> {
  const { data, error } = await db
    ...
    .select("id, display_name, contact_email, best_contact_kind, invite_code, code_sent_at, planned_wallet, planned_wallets_detail")
    .eq("id", testerId)
    .maybeSingle();
...
export async function sendDraftAction(
  ...
  revalidatePath("/admin/beta-v2/drafts");
  revalidatePath("/admin/beta-v2/sent");
  revalidatePath(`/admin/beta-v2/drafts/${testerId}`);
  revalidatePath(`/admin/beta-v2/sent/${testerId}`);
  return { ok: true };
}
