...
import { contactValueFor, listDraftTesters } from "@/lib/beta/drafts";
  ...
  if (drafts.length === 0) {
    return (
      <section className="rounded-md border border-zinc-800 bg-zinc-900/40 p-8 text-center text-sm text-zinc-400">
        No pending invite drafts. Any applicant whose best contact is email and
        hasn't been sent yet will appear here.
      </section>
    );
  }
  ...
  return (
    <section className="overflow-hidden rounded-md border border-zinc-800">
      <table className="w-full text-left text-sm">
        ...
        <tbody>
          {drafts.map((d) => {
            const hasSdk = d.focus_areas.includes("sdk");
            ...
            return (
              ...
              >
                ...
                <td className="px-3 py-2 text-right">
                  <Link
                    href={`/admin/beta/drafts/${encodeURIComponent(d.id)}`}
