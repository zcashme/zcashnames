...
import { listSentTesters } from "@/lib/beta/drafts";
  ...
  if (rows.length === 0) {
    return (
      <section className="rounded-md border border-zinc-800 bg-zinc-900/40 p-8 text-center text-sm text-zinc-400">
        Nothing sent yet. Invites you send from the Drafts view will appear
        here.
      </section>
    );
  }
          ...
          {rows.map((r) => (
            <tr key={r.id} className="border-t border-zinc-800 hover:bg-zinc-900/60">
              ...
              <td className="px-3 py-2 text-zinc-300">
                {r.best_contact_kind === "email"
                  ? r.contact_email
                  : (r.best_contact_kind ?? "—")}
              </td>
              <td className="px-3 py-2 text-zinc-400">
                {r.code_sent_at
                  ? new Date(r.code_sent_at).toLocaleString()
                  : "—"}
              </td>
              <td className="px-3 py-2">{statusBadge(r.notice_status)}</td>
              <td className="px-3 py-2 text-right">
                <Link
                  href={`/admin/beta/sent/${encodeURIComponent(r.id)}`}
