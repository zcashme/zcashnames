...
import { getDraft } from "@/lib/beta/drafts";
...
export default async function SentDetailPage({
  ...
  const { data, error } = await db
    .from("beta_testers")
    .select(
      "id, display_name, contact_email, best_contact_kind, invite_code, code_sent_at, status, notice_status, notice_error, notice_attempted_at",
    )
    .eq("id", id)
    .maybeSingle();
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="text-xs text-zinc-500">
        <Link href="/admin/beta/sent" className="text-amber-400 hover:text-amber-300">
          ← Back to sent
        </Link>
      </div>
      ...
      <header className="flex flex-wrap items-baseline justify-between gap-2">
        ...
        <span className="text-xs text-zinc-400">
          ...
          <span className="text-zinc-200">
            {new Date(tester.code_sent_at).toLocaleString()}
          </span>
