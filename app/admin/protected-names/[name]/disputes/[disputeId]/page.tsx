...
export default async function ProtectedNameDisputePage({ params }: PageProps) {
  ...
  if (dispute.protected_name.toLowerCase() !== name.toLowerCase()) {
    notFound();
  }
  const decisions = await listProtectedNameDecisions("dispute", dispute.id);

  const detail = await getProtectedNameDetail(nameRow.name);
  const isParent = !nameRow.parent_name;
  const variantCount = detail?.variants.length ?? 0;
  const canTransitionLikely =
    nameRow.status === dispute.name_status_at_submission;

  return (
    <div className="flex flex-col gap-4">
      <div>
        <Link
          href={`/admin/protected-names/${encodeURIComponent(nameRow.name)}`}
      ...
      <section className="grid gap-4 rounded-md border border-zinc-800 bg-zinc-950/40 p-4 lg:grid-cols-2">
        ...
        <Field
          label="Status at submission"
          value={dispute.name_status_at_submission}
        />
        <div>
        ...
        <Field
          ...
          value={dispute.submitted_by_email ?? "—"}
        />
        <Field
          label="Preferred contact"
          value={
            dispute.preferred_contact_kind
              ? `${dispute.preferred_contact_kind}: ${dispute.preferred_contact_value ?? ""}`
              : "—"
          }
        />
        <Field
          label="Zcash UA"
          value={dispute.zcash_unified_address ?? "—"}
          mono
        />
