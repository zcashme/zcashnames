...
import { CAMPAIGN_SERIES } from "@/lib/campaigns/types";

export const DEFAULT_EMAIL_SERIES = "general";
...
export async function listActiveSubscriberEmails(series: string): Promise<Set<string>> {
  const { data, error } = await db
    .from("email_subscribers")
    .select("email")
    .eq("series", series)
...
export async function listSubscriberPreferences(
  ...
  const effectiveSeries = (seriesList && seriesList.length > 0 ? seriesList : CAMPAIGN_SERIES).map(
    (series) => series.trim(),
  );
...
export async function unsubscribeAll(email: string, seriesList?: string[]): Promise<number> {
  ...
  const effectiveSeries =
    seriesList && seriesList.length > 0 ? seriesList : [...CAMPAIGN_SERIES, "waitlist"];
...
export async function applySubscriberPreferences(args: {
  ...
  for (const rawSeries of args.seriesList) {
    const series =
      rawSeries === "launch" ? "users" : rawSeries === "updates" ? "waitlist" : rawSeries;
    const desired = Boolean(args.desiredSeries[series] || args.desiredSeries[rawSeries]);
    if (series === "waitlist") {
      if (desired) {
