...
export type SvgCaptchaChallenge = {
  /** data:image/svg+xml;base64,... — ready for <img src>. */
  image: string;
  /**
   * `${expiresMs}.${nonce}.${hmac}` — HMAC binds the normalized answer
...
function getSecret(): string {
  ...
  if (!secret) throw new Error("Missing captcha secret.");
  return secret;
}
...
export function createSvgCaptchaChallenge(now = Date.now()): SvgCaptchaChallenge {
...
export function verifySvgCaptcha({
  token,
  answer,
  now = Date.now(),
}: {
