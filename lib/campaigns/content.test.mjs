...
test("parses aligned paragraphs without a box or heading", () => {
  const blocks = parseEmailContent(
    [
      "Hello",
      "center: Terms · FAQs",
      "justify: A longer footer line.",
      "left: Explicit left line.",
    ].join("\n"),
  );
  assert.deepEqual(blocks, [
    { type: "paragraph", text: "Hello", align: "left" },
    { type: "paragraph", text: "Terms · FAQs", align: "center" },
    { type: "paragraph", text: "A longer footer line.", align: "justify" },
    { type: "paragraph", text: "Explicit left line.", align: "left" },
  ]);
});

test("does not treat heading alignment as a paragraph", () => {
  const blocks = parseEmailContent("### center: Reserve your name");
  assert.deepEqual(blocks, [
    { type: "heading", level: 3, text: "Reserve your name", align: "center" },
  ]);
});

test("resolves the direct waitlist reservation URL token", () => {
  assert.equal(
    resolveCampaignTokens("Reserve here: {{reserve_url}}", {
      reserveUrl: "https://zcashnames.com/reserve?token=sample-token",
      referralStats: null,
    }),
    "Reserve here: https://zcashnames.com/reserve?token=sample-token",
  );
});
