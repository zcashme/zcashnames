      ...
      <header className="rounded-md border border-zinc-800 bg-zinc-900/40 p-4">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-[1fr_auto_auto]">
          ...
          <div className="rounded-md border border-zinc-800 bg-zinc-950/40 px-3 py-2 text-xs text-zinc-400">
            ...
            {tokensExpanded ? (
              <>
                <div className="mt-2 break-words">{`{{name}} {{referral_code}} {{referral_url}} {{dashboard_url}} {{human_referral_code}} {{human_referral_url}} {{human_dashboard_url}} {{confirm_response_url}} {{reserve_url}} {{other_interested_count}} {{beta_display_name}} {{beta_invite_code}} {{beta_invite_link}} {{direct_referrals}} {{indirect_referrals}} {{attributed_referrals}} {{referrals_24h_count}} {{referrals_24h_growth_pct}} {{referrals_7d_count}} {{referrals_7d_growth_pct}} {{referrals_30d_count}} {{referrals_30d_growth_pct}} {{depth_1_referrals}} {{depth_2_referrals}} {{depth_3_referrals}} {{leaderboard_rank}} {{waitlist_position}} {{waitlist_total}} {{max_referral_depth}} {{potential_rewards}} {{root_badge}} {{commission_unlocked}} {{referrals_unlocked}}`}</div>
                <div className="mt-2 text-[11px] text-zinc-500">
                  <code>{"{{reserve_url}}"}</code> links directly to the recipient&apos;s signed reservation page without recording campaign confirmation. <code>{"{{other_interested_count}}"}</code> shows how many other waitlist inboxes requested the same exact name. Beta invite tokens resolve only when the recipient email matches <code>beta_testers_v2.contact_email</code>. Referral, dashboard, and stat tokens are meaningful only for <code>zn_waitlist</code>. Scheduled sends refresh stat values at actual send time.
                </div>
              </>
            ) : null}
          </div>
