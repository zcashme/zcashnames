...
export default function DecisionEmailPreview({ name, workflow, decision, reason, nameStatus, didTransition, submittedReason, isCorrection, isDecisionCorrection }: { name: string; workflow: string; decision: "approved" | "denied"; reason: string; nameStatus?: string | null; didTransition?: boolean; submittedReason?: string | null; isCorrection?: boolean; isDecisionCorrection?: boolean }) {
  ...
  function toggle() { if (open) { setOpen(false); return; } setError(null); startTransition(async () => { try { setPreview(await renderProtectedNameDecisionPreviewAction({ name, workflow, decision, reason, nameStatus, didTransition, submittedReason, isCorrection, isDecisionCorrection })); setOpen(true); } catch (previewError) { setError(previewError instanceof Error ? previewError.message : "Could not render email preview."); } }); }
