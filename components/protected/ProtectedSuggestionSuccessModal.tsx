...
type ProtectedSuggestionSuccessModalProps = {
  isOpen: boolean;
  onClose: () => void;
  name: string;
};

export default function ProtectedSuggestionSuccessModal({
  isOpen,
  onClose,
  name,
}: ProtectedSuggestionSuccessModalProps) {
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4 px-8 pb-8 pt-12 text-center">
          ...
          <p className="text-sm" style={{ color: "var(--fg-body)", lineHeight: 1.6 }}>
            Suggestions are reviewed on a rolling basis.
            <br />
          ...
          <div className="relative z-20 grid w-full grid-cols-1 justify-items-center gap-3 sm:grid-cols-2">
            <ProtectedSuggestionShareButton
              mode="success"
              submittedName={name}
            ...
            />
            <Link
              href="/protected?underReviewOnly=true"
