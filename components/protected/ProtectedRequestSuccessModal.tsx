...
import ProtectedRequestShareButton from "@/components/protected/ProtectedRequestShareButton";

type ProtectedRequestSuccessModalProps = {
  isOpen: boolean;
  onClose: () => void;
  name: string;
...
export default function ProtectedRequestSuccessModal({
  isOpen,
  onClose,
  name,
  referenceNumber,
}: ProtectedRequestSuccessModalProps) {
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4 px-8 pb-8 pt-12 text-center">
          ...
          <p className="text-sm" style={{ color: "var(--fg-body)", lineHeight: 1.6 }}>
            ...
            <br />
            Approval does not waive the purchase price.
          </p>
          {referenceNumber ? (
