...
A single-page glossary for Zcash Names and ZNS. When the docs use a term that has a precise meaning, it lives here.

## Identity

**Zcash Names** is the brand: the user-facing experience at [zcashnames.com](https://www.zcashnames.com), the wallet integrations, the marketplace UI.

**ZNS** (Zcash Name Service) is the protocol: the on-chain memo grammar, the indexer, the JSON-RPC API, the SDK ports. Zcash Names is built on ZNS.
...
**`.zcash` and `.zec` suffixes** are display-only labels used by the website and wallets. They are interchangeable and never stored on chain. The SDKs strip them automatically — `resolveName("Alice.zcash")` resolves `alice` — but they must never appear in a memo or a raw RPC query.
...
**Protected name** is a name flagged in the operator's protected-names policy (`zn_protected_names` with `status = protected`, not redeemed, and not past optional `expires_at`). Requires an unlock code from the maintainers to claim. See [Protected Names](/docs/use/protected-names).

**Unlock code** is the deterministic HMAC fingerprint the signing service issues to redeem a protected name.

## Read next

- [What is Zcash Names?](/docs/learn/what-is-zns) - the friendly intro
