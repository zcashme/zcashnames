...
**Protected name** is a name flagged in the operator's protected-names policy (`zn_protected_names` with `status = protected`, not redeemed, and not past optional `expires_at`). Requires an unlock code from the maintainers to claim. See [Protected Names](/docs/use/reserved-names).
